...
constexpr const char *kNvidiaFsDevice = "/dev/nvidia-fs0";
...
std::string md5OnPeer(const std::string &Remote) {
  auto Ran = runLocal({"ssh", "-o", "LogLevel=ERROR", peerHost(), "md5sum", Remote});
  return Ran && Ran->ExitStatus == 0 ? Ran->Output.substr(0, 32) : std::string{};
}
...
long nvidiaFsCount(const std::string &Counter) {
  ...
  if (Count == std::string::npos || Count > Stats.find('\n', Line)) return -1;
...
public:
  ...
  ssize_t read(void *Device, size_t Size, off_t FileOffset, off_t DeviceOffset) { return cuFileRead(Handle, Device, Size, FileOffset, DeviceOffset); }
...
protected:
  void SetUp() override {
    ...
    ASSERT_FALSE(std::filesystem::exists("/sys/module/nvidia_fs")) << "the GDS suite needs exclusive nvidia-fs module ownership";
    const char *Module = ::getenv("RAIL_NVFS_KO");
    ASSERT_NE(Module, nullptr) << "set RAIL_NVFS_KO to the patched nvidia-fs.ko";
    ASSERT_TRUE(std::filesystem::exists(modulePath()));
    ...
    ASSERT_TRUE(loadModule(Module));
    NvidiaFsLoaded = true;
    ASSERT_TRUE(writeWholeFile("/sys/module/nvidia_fs/parameters/rw_stats_enabled", "1"));
    ASSERT_TRUE(std::filesystem::exists(kNvidiaFsDevice));
  ...
  // Unload nvidia-fs first to release its reference on railfs.
  void TearDown() override {
    if (NvidiaFsLoaded) {
      RecordProperty("nvfs_reads", nvidiaFsCount("Reads"));
      RecordProperty("nvfs_writes", nvidiaFsCount("Writes"));
    }
    if (DriverOpen) cuFileDriverClose();
    if (Loaded && mounted()) EXPECT_TRUE(unmountFilesystem(Mountpoint));
    if (NvidiaFsLoaded) EXPECT_TRUE(unloadModule("nvidia_fs"));
    Kernel::TearDown();
  }
...
TEST_F(Gds, ReadsFromThePeerIntoGpuMemory) {
  ...
  // Correct bytes alone cannot distinguish GDS from CPU fallback.
...
TEST_F(Gds, ShortReadPreservesTheBufferTail) {
  constexpr size_t FileSize = (1u << 20) + 4096;
  constexpr size_t BufferSize = 2u << 20;
  constexpr uint32_t Seed = 15;
  seedPattern("gpu-short.bin", FileSize, Seed);

  DeviceBuffer Buffer(BufferSize);
  ASSERT_TRUE(Buffer.ok());
  fillOnDevice(Buffer.Ptr, BufferSize, Seed ^ 0xFFu, 0);

  GpuFile File(Mountpoint + "/gpu-short.bin", O_RDONLY);
  ASSERT_TRUE(File.ok()) << File.Error;
  ASSERT_EQ(File.read(Buffer.Ptr, BufferSize, 0, 0), static_cast<ssize_t>(FileSize));
  EXPECT_EQ(mismatchesOnDevice(Buffer.Ptr, FileSize, Seed, 0), 0u);
  const auto *Tail = static_cast<const uint8_t *>(Buffer.Ptr) + FileSize;
  EXPECT_EQ(mismatchesOnDevice(Tail, BufferSize - FileSize, Seed ^ 0xFFu, FileSize), 0u);
}

TEST_F(Gds, ReadAtEofLeavesTheBufferUntouched) {
  constexpr size_t Size = 4096;
  constexpr uint32_t Seed = 16;
  seedPattern("gpu-eof.bin", Size, Seed);

  DeviceBuffer Buffer(Size);
  ASSERT_TRUE(Buffer.ok());
  fillOnDevice(Buffer.Ptr, Size, Seed ^ 0xFFu, 0);

  GpuFile File(Mountpoint + "/gpu-eof.bin", O_RDONLY);
  ASSERT_TRUE(File.ok()) << File.Error;
  EXPECT_EQ(File.read(Buffer.Ptr, Size, Size, 0), 0);
  EXPECT_EQ(mismatchesOnDevice(Buffer.Ptr, Size, Seed ^ 0xFFu, 0), 0u);
}

TEST_F(Gds, DisabledGdsRejectsGpuReads) {
  constexpr size_t Size = 4096;
  constexpr uint32_t Seed = 17;
  seedPattern("gpu-disabled.bin", Size, Seed);

  DeviceBuffer Buffer(Size);
  ASSERT_TRUE(Buffer.ok());
  fillOnDevice(Buffer.Ptr, Size, Seed ^ 0xFFu, 0);
  GpuFile File(Mountpoint + "/gpu-disabled.bin", O_RDONLY);
  ASSERT_TRUE(File.ok()) << File.Error;
  ASSERT_TRUE(writeWholeFile(kGdsSwitch, "N"));

  EXPECT_LT(File.read(Buffer.Ptr, Size, 0, 0), 0);
  EXPECT_EQ(mismatchesOnDevice(Buffer.Ptr, Size, Seed ^ 0xFFu, 0), 0u);
}

TEST_F(Gds, VerifiedMountRejectsGpuReads) {
  ASSERT_TRUE(unmountFilesystem(Mountpoint));
  ASSERT_TRUE(mountIt(defaultOptions() + ",rdma"));
  constexpr size_t Size = 4096;
  constexpr uint32_t Seed = 18;
  seedPattern("gpu-verified.bin", Size, Seed);

  DeviceBuffer Buffer(Size);
  ASSERT_TRUE(Buffer.ok());
  fillOnDevice(Buffer.Ptr, Size, Seed ^ 0xFFu, 0);
  GpuFile File(Mountpoint + "/gpu-verified.bin", O_RDONLY);
  ASSERT_TRUE(File.ok()) << File.Error;
  EXPECT_LT(File.read(Buffer.Ptr, Size, 0, 0), 0);
  EXPECT_EQ(mismatchesOnDevice(Buffer.Ptr, Size, Seed ^ 0xFFu, 0), 0u);
}

TEST_F(Gds, WritesSynchronouslyFromGpuMemoryToThePeer) {
  ...
  ASSERT_GE(Before, 0);
  {
    GpuFile File(Mountpoint + "/gpu-write.bin", O_WRONLY | O_DSYNC);
    ASSERT_TRUE(File.ok()) << File.Error;
